...
@register_model
def MIL_VT_small_patch32_768(pretrained=False, **kwargs):
    model = MILVisionTransformer(
        img_size=768, patch_size=32, num_classes=1, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
